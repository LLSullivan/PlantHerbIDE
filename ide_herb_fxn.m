function [speed_inst_s,speed_inst_a,xleft_s,xleft_a,xright_s,xright_a] = ide_herb_fxn(kflag,nu,sigma_s,sigma_a,gamma,phi,mu_g,mu_a,mu_s,delta,gflag)
% USAGE: [speed_inst_s,speed_inst_a,xleft_s,xleft_a,xright_s,xright_a] = ide_herb_fxn(kflag,nu,sigma_s,sigma_a,gamma,phi,mu_g,mu_a,mu_s,delta,gflag)
%
% inputs: 
...
%
%
% last updated: 23 Mar 2023 (by Allison K Shaw)
...
%-----PARAMETERS----------------------------------------------------------%
  ...
  %Dispersal kernel
  if kflag == 1
      k = normpdf(x2,0,sqrt(nu + delta)); % gaussian kernel
  elseif kflag == 2
      k = exp(-sqrt((2*(x2).^2)/(nu + delta)))./sqrt(2*(nu + delta)); % laplace kernel
  else
...
if gflag==1
    ...
    ylabel('wave speed')
    legend('seed','adult')
    saveas(100,strcat(['speed_kflag=' num2str(kflag) '_nu=' num2str(nu) '_sigmas=' num2str(sigma_s) '_sigmaa=' num2str(sigma_a) '_gamma=' num2str(gamma) '_phi=' num2str(phi) '_mug=' num2str(mu_g) '_mua=' num2str(mu_a) '_mus=' num2str(mu_s) '_delta=' num2str(delta) '.jpg']))
    ...
    title(['t = ',num2str(j)]);
    legend('seed','adult')
    saveas(101,strcat(['distrbution_kflag=' num2str(kflag) '_nu=' num2str(nu) '_sigmas=' num2str(sigma_s) '_sigmaa=' num2str(sigma_a) '_gamma=' num2str(gamma) '_phi=' num2str(phi) '_mug=' num2str(mu_g) '_mua=' num2str(mu_a) '_mus=' num2str(mu_s) '_delta=' num2str(delta) '.jpg']))
end
