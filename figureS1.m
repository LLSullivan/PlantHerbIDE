...
load results_simulated_laplace.mat
load results_analytic_laplace.mat
...
axes('position',[sx sy+h+dy w h])
    ...
    plot(mu_s_vec_ana,cstar_all_1,'k-','LineWidth',lw_lines)
    box on
...
axes('position',[sx+w+dx sy+h+dy w h])
    ...
    plot(mu_a_vec_ana,cstar_all_2,'k-','LineWidth',lw_lines)
    box on
    axis([0 1.02 -0.02 0.8])
...
axes('position',[sx sy w h])
    line([-0.6 0.2],[speed_basic_s speed_basic_s],'color',g1,'LineStyle','--','LineWidth',lw_lines)
    hold on
    plot(delta_vec_neg,speed_inst_s_all_3(:,end),'k.','MarkerSize',mksize)
    plot(delta_vec_neg_ana,cstar_all_3,'k-','LineWidth',lw_lines)
    box on
